...
export function Kbd({ className, ...props }: React.ComponentProps<"kbd">) {
    return (
        <kbd
            className={cn(
                "pointer-events-none inline-flex h-5 min-w-5 select-none items-center justify-center gap-1 rounded-full bg-card/50 px-1.5 font-medium font-sans text-muted-foreground text-xs uppercase [&_svg:not([class*='size-'])]:size-3",
                className
            )}
            data-slot="kbd"
            {...props}
